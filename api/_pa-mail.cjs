...
const buildRawMessage = ({ to, cc = [], subject, body, messageType, replyHeaders = {}, attachments = [], config = mailConfig() }) => {
    ...
    const carbonCopy = (Array.isArray(cc) ? cc : String(cc || "").split(","))
        ...
        .filter((value, index, values) => value.toLowerCase() !== recipient.toLowerCase()
            && value.toLowerCase() !== OFFICIAL_EMAIL && values.findIndex((item) => item.toLowerCase() === value.toLowerCase()) === index);
    if (carbonCopy.length > 30) throw new Error("invalid_reply_cc");

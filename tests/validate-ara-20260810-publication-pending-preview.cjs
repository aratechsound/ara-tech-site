...
assert.match(adminHtml, /<option value="pending">公開待ち候補として保存<\/option>/u);
assert.match(adminJs, /const isCreatingPendingCandidate = \(\) => !editingPost && publicationMode\.value === 'pending';/u);
assert.match(adminJs, /const pendingCandidate = isPendingCandidate\(editingPost\) \|\| isCreatingPendingCandidate\(\);/u);
assert.match(adminJs, /lifecycleStatus === 'upcoming' && \(isPublishing \|\| pendingCandidate\)/u);
...
assert.match(adminJs, /use_image_on_public_page: pendingUsesStoredImage/);
assert.match(adminJs, /resolveImageConfirmation\(/);
...
assert.match(previewHtml, /公開待ち一覧で確認中の画像です。「公開する」で公開ページに掲載されます。/);
const omittedImagePreviewHtml = workHandler.renderWorkPage({ ...previewPost, image_usage_status: 'not_permitted' }, { preview: true });
assert.match(omittedImagePreviewHtml, /管理画面の確認用画像です。現在は「画像を掲載しない」設定です。/);

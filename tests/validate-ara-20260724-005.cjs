...
for (const file of publicPages) {
    assert.match(read(file), /site-navigation\.css\?v=ara-20260724-005/, `${file} must load shared public header CSS`);
}
...
assert.match(navigationCss, /\.navbar \.nav-link,[\s\S]*?background:\s*#005bb9/);
...
assert.match(workApi, /site-navigation\.css\?v=ara-20260724-005/);

...
(async () => {
  ...
  try {
    ...
    assert.equal(issuedOffer.snapshot.case.event_time, '10:00〜15:00');
    assert.equal(issuedOffer.snapshot.estimate.amount_minor, 110000);
    ...
    const contractService = createContractService({ fetchImpl: fixture.fetchImpl });
    const acceptedResult = await contractService.accept({ token, offer_id: offerId, snapshot_sha256: offer.snapshot_sha256, confirmer_name: 'HTTP Fixture Customer', agree: true });
    assert.equal(acceptedResult.state, 'accepted'); assert.equal(acceptedResult.receipt_status, 'ready'); assert.equal(acceptedResult.email_status, 'sent');
    const receiptJobs = (await fixture.db.query("select * from public.pa_commercial_outbox where job_kind='accept_receipt' and aggregate_id=$1", [offerId])).rows;
    assert.equal(receiptJobs.length, 1); assert.equal(receiptJobs[0].state, 'sent'); assert.equal(receiptJobs[0].subject, '【ARA-TECH】正式受注確認完了｜龍姫湖まつり2026（検証用）');
    const automaticReceiptMime = Buffer.from(fixture.state.lastRaw.raw, 'base64url').toString('utf8');
    assert.match(automaticReceiptMime, /Content-Type: text\/plain; charset=UTF-8/); assert.match(automaticReceiptMime, /Content-Type: text\/html; charset=UTF-8/);
    assert.equal((automaticReceiptMime.match(/Content-Disposition: attachment/gu) || []).length, 1);
    const attachments = await contractService.receiptAttachments(fixture.inquiryId, offerId);
    assert.equal(attachments.length, 1); assert.match(attachments[0].filename, /^正式受注確認書_/);
    const receipt = await contractService.customerReceipt(token); assert.equal(receipt.sha256, sha(receipt.bytes)); assert.equal((await PDFDocument.load(receipt.bytes)).getPageCount(), 3);
    const replay = await contractService.accept({ token, offer_id: offerId, snapshot_sha256: offer.snapshot_sha256, confirmer_name: 'HTTP Fixture Customer', agree: true });
    assert.equal(replay.already_received, true); assert.equal((await fixture.db.query("select count(*)::int n from public.pa_commercial_outbox where job_kind='accept_receipt' and aggregate_id=$1", [offerId])).rows[0].n, 1);

...
(async () => {
  ...
  try {
    ...
    assert.equal(issuedOffer.snapshot.case.event_time, '10:00〜15:30');
    ...
    assert.equal((automaticReceiptMime.match(/Content-Disposition: attachment/gu) || []).length, 2);
    const attachments = await contractService.receiptAttachments(fixture.inquiryId, offerId);
    assert.equal(attachments.length, 2); assert.match(attachments[0].filename, /^正式受注確認書_/); assert.equal(attachments[1].filename, 'http-estimate.pdf');
    const receipt = await contractService.customerReceipt(token); assert.equal(receipt.sha256, sha(receipt.bytes));

 ...
 try{
  ...
  server=http.createServer(async(req,res)=>{
   try{
    ...
    if(url.pathname==='/admin-test.js'){res.writeHead(200,{'Content-Type':'text/javascript'});res.end(`import {renderContractPanel} from '/js/pa-contract-admin.js';const item={id:'${f.inquiryId}',event_date:'2026-10-18',customer_name:'管理下テスト',request_summary:'テストPA'};renderContractPanel({case:item,progress:{estimate_amount:170500},getAccessToken:async()=>'fixture-admin',getCurrentCase:()=>item});`);return;}
    if(url.pathname==='/admin-test.html'){res.writeHead(200,{'Content-Type':'text/html',...headersFor('/pa-admin.html')});res.end('<!doctype html><html lang="ja"><meta charset="utf-8"><link rel="stylesheet" href="/pa-admin.css"><script type="module" src="/admin-test.js"></script><main><section id="formal-contract-panel"></section></main></html>');return;}

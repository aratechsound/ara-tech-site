...
async function createFixture(){
 ...
 alter table public.pa_email_deliveries add column inquiry_id uuid, add column status text, add column gmail_thread_id text, add column gmail_message_id text, add column message_type text, add column sent_at timestamptz, add column recipient text, add column subject text;
  ...
  update public.pa_inquiries set email='customer@example.invalid',customer_name='管理下テスト担当者',contact_name='管理下テスト担当者',organization_name='テスト実行委員会',event_name='龍姫湖まつり2026（検証用）',event_date='2026-10-18',event_time='10:00〜15:30',venue='管理下テスト会場',inquiry_number='PA-20260907-00001',request_summary='屋外イベントPA・設営・本番対応';

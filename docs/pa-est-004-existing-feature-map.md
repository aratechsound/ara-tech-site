...
既存の`pa_payment_records`を削除せず利用し、snapshotで既存`amount`をV5表示用`amount_minor`へ正規化する。請求前の前払いも同じ台帳へappend-onlyで記録し、請求作成時に原子的に紐付ける。既存のcontract/token/receiptは保護する。メール行ごとの旧「この送信を見積提出として記録」は除き、案件単位の復旧dialogへ集約した。変更提案と再案内は既存正式受注を上書きせず、別のseries/job kindで追加した。

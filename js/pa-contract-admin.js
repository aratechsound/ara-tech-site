...
export function renderContractPanel(context){
 ...
 if(active?.id===context.case.id&&active.caseRef===context.case&&active.progressRef===context.progress){if(active.gmailRef!==context.gmailRef){active.gmailRef=context.gmailRef;active.refresh();}return;}
 ...
 active.refresh=()=>refresh().catch(error);
 refresh().catch(error);
}
